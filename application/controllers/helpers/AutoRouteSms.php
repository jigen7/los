                ...
                Zend_Controller_Action_Helper_Abstract
{
	function direct($capno){
		...
		//check if the account is CMGH/PRES
		if($nextRole == "CMGH/PRES"){
			//CSH - ReAp
			if(strpos($detail->routetag,'CMGH') !== false){
				$nextRole = "CMGH";	
			}
			else if(strpos($detail->routetag,'PRES') !== false){
				$nextRole = "PRES";	
			}
		}
		...
		if($nextApproverName == "CRECOM" || $nextApproverName == "SUBCRECOM" || $nextApproverName == "BOD"){
			...
			if($nextApproverName == "CRECOM"){
				...
				foreach($enableMember as $y){
					...
					if($crecomModel->chkifDecide($detail->capno,$y->role) === false){
						...
						if($detailx){
						$detailxArray = $detailx->toArray();
						$detailxArray['id'] = null;
						$detailxArray['decided'] = "T";
						$detailxArray['delete_from']= "Crecom Member Decided Move";
						$detailxArray['date_moved'] = date("r");  
						$smsHistory->insert($detailxArray);
						$where = "id =".$detailx->id;
						$sms->delete($where);
						}
					}
			...
			foreach($rows as $x){
				...
				if($x->approver != $nextApproverName){
					...
					$arr['id'] = null;
					$arr['decided'] = "T";
					$arr['delete_from']= "Approver is not equal to the Current approver";
					$arr['date_moved'] = date("r");  
					$smsHistory->insert($arr);
					$where = "id =".$x->id;
					$sms->delete($where);
				}
			...
			*/
			...
			if($statusTable->isDecided($account_status) === true){
				foreach($rows as $x){
					...
					$arr['id'] = null;
					$arr['decided'] = "T";
					$arr['delete_from']= "Account Already Decided";
					$arr['date_moved'] = date("r");  
					$smsHistory->insert($arr);
					$where = "id =".$x->id;
					$sms->delete($where);
