...
/***********************MODULE 2****************************/	
	...
	function chkApprovalStatus2($capno,$grp){
		...
		if($accountTable->chkChairmanifDecidedNormal($capno) > 0){
			...
			if($sumDecision == 3){
				if($approve > $disapprove){
					if(strpos($routetag,'-CRECOM')!== false){
					$status = ucfirst(strtolower($group)).' - Ap';
					$decide = 'Approved';
					}
					if(strpos($routetag,'-EXE-BOD')!== false || 
					strpos($routetag,'CRECOM-EXEBOD')!== false){
					$status = ucfirst(strtolower($group)).' - ENEXEBOD';
					$decide = 'Approved BOD Confirmation';
					}
				}
				else if($disapprove > $approve){
					$status = ucfirst(strtolower($group)).' - R';
					$decide = 'Rejected';
				}			
		...
		if($chkStatus != $status){
			...
			$data = array(
			'capno'=>$capno,
			'decision'=>ucfirst(strtolower($group)).' '.$decide,
			'approved_by'=>ucfirst(strtolower($group)),
			'reason'=>'',
			'date_approval'=>date("m-d-Y h:i a"),
			'date_approval2'=>date("r"),
			'role'=> '',
