...
/*****************************CMGH APPROVAL LEVEL*****/
		...
		*/
		...
		$select = $table->select();
		$select->where("account_status like 'ALMH - ENCMGH' OR account_status = 'CSH - P' OR account_status = 'CSH - ReAp' OR account_status = 'CO - ReAp - ABCSH' OR account_status = 'CO - ReR - ABCSH' OR account_status = 'CO - P - ABCSH'");
		$select->where("routetag like '%CMGH%' OR routetag like '%$username%' OR routetag like '%-CRECOM%' OR routetag like '%-EXE-BOD%'");
		$result = $table->fetchAll($select);
		//Submitted
		$select = $table->select();
		$select->where("account_status = 'CMGH - P' OR account_status = 'CMGH - Ap' OR account_status = 'CMGH - R' OR account_status = 'CMGH - ReR' OR account_status = 'CMGH - ReAp'");
...
/*****************************PRES APPROVAL LEVEL*****/
		...
		//EXEBOARD LEVEL
			...
			$select = $table->select();		
			$select->where(arrayString($condition));
			$select->where("routetag like '%EXE-BOD%' OR routetag like '%EXEBOD%'");
			$select->order("date_decided DESC");
...
/*****************************CORPSEC ENDORSEMENT LEVEL*****/
		...
		//Submitted
			$select = $table->select();
			$select->orwhere('account_status like ?','Booked');
			foreach($statusTable->routeBox('pres_decided') as $x){
			$select->orwhere('account_status like ?',$x->status);
			}
			$condition = $select->getPart(Zend_Db_Select::WHERE);
    ...
    public function addaccountAction()
    {
		...
		/*******************************/
	     ...
	     $formData = $this->getRequest()->getPost();
			...
			if($comaker_of){			
			$account_status = 'none';// no account status for comaker
				}
			else{
	...
	if ($this->getRequest()->isPost()) {
	    ...
	    if ($form->isValid($formData)) {
			...
			if($formData['save_loan']){
				...
				$existing_auto = getExistLoan($capno,'totalvalue','Auto','CBCS');
				$existing_housing = getExistLoan($capno,'totalvalue','Home','CBCS');
				$existing_personal = getExistLoan($capno,'totalvalue','Personal','CBCS');
				$existing_business = getExistLoan($capno,'totalvalue','Business','CBCS');
				$existing_other = getExistLoan($capno,'totalvalue','Others','CBCS');		
				$total_bankproper= $existing_auto + $existing_housing + $existing_personal + $existing_business + $existing_other;						
			
			$data = array(
			'amountloan'=>moneyconvert($form->getValue('amountloan')),
			//Sept 13,2010 Aggretage Exposure 
			'amountloan2'=>moneyconvert($total_bankproper+$form->getValue('amountloan')),
...
public function searchAction(){
    ...
    $formData = $this->getRequest()->getPost();
		...
		$select = $table->select();
			...
			$select->where("date_decided between '$startdate'  and '$enddate' OR application_date between '$startdate' and '$enddate'");
			...
			if($decision){
				...
				$statusTable = new Model_Admin_AccountStatus();
				$selectApprove = $table->select();
				foreach($statusTable->routeBox('approve') as $x){
				$selectApprove->orwhere('account_status like ?',$x->status);
				}
				$condition = $selectApprove->getPart(Zend_Db_Select::WHERE);
				$select->where(arrayString($condition));		
				}
				else if($decision == 'Rejected'){
				$statusTable = new Model_Admin_AccountStatus();
				$selectReject = $table->select();

				foreach($statusTable->routeBox('reject') as $x){
				$selectReject->orwhere('account_status like ?',$x->status);
				}
				$condition = $selectReject->getPart(Zend_Db_Select::WHERE);
				$select->where(arrayString($condition));	
				}
...
public function masubmitAction(){
			...
			if ($this->getRequest()->isPost()) {
				$formData = $this->getRequest()->getPost();
					...
					$score_orig = $this->_helper->$s($capno,'ocs-off');
					$score_tag_orig = $this->view->viewScore($score_orig,getHighest($capno),'score');
					$score_orig_final = $score_orig[0];
					...
					'score_orig'=>$score_orig_final,
					'score_tag_orig'=>$score_tag_orig,
					);
					$where = "capno like '$capno'";
					...
					'remarks_veh_use_taxi_downpayment'=>'',				
					'remarks_veh_use_taxi_term'=>'',				
					...
					'remarks_loan_amount'=>$form->getValue('loan_amount'),
					...
					'remarks_veh_use_taxi_downpayment' =>$form->getValue('dev_veh_use_taxi_downpayment'),
					'remarks_veh_use_taxi_term' =>$form->getValue('dev_veh_use_taxi_term'),
...
public function submitAction(){
	...
	// Function for the CA Submit of the Deviation Box
			...
			$form->dev_veh_use_taxi_downpayment->setValue($deviationList->remarks_veh_use_taxi_downpayment);
			$form->dev_veh_use_taxi_term->setValue($deviationList->remarks_veh_use_taxi_term);
			...
			if ($this->getRequest()->isPost()) {
				$formData = $this->getRequest()->getPost();
					...
					'remarks_loan_amount'=>stripslashes($form->getValue('loan_amount')),
					...
					'remarks_veh_use_taxi_downpayment' =>stripslashes($form->getValue('dev_veh_use_taxi_downpayment')),
					'remarks_veh_use_taxi_term' =>stripslashes($form->getValue('dev_veh_use_taxi_term')),
...
public function mabooksubmitAction(){
			...
			if ($this->getRequest()->isPost()) {
				$formData = $this->getRequest()->getPost();
					...
					$history->insert($hdata);
					//End of History
					$this->_helper->Accounts($capno,'MA - PreB');
...
public function lartsmabooksubmitAction(){
			...
			if ($this->getRequest()->isPost()) {
				$formData = $this->getRequest()->getPost();
					...
					$history->insert($hdata);
					$this->_helper->Accounts($capno,'LA - RTMA');
...
public function lortslabooksubmitAction(){
			...
			if ($this->getRequest()->isPost()) {
				$formData = $this->getRequest()->getPost();
					...
					$history->insert($hdata);
					$this->_helper->Accounts($capno,'LO - RTLA');
			...
			if ($this->getRequest()->isPost()) {
				$formData = $this->getRequest()->getPost();
					...
					if($action == 'approval'){
						...
						//retain routetag if Crecom approval higher for pending
						if(strpos($route,'-CRECOM')!== false || strpos($route,'-EXE-BOD') !== false){ 
					...
					$this->_helper->UpdateAcntStatus->colevel($capno);
					$this->_helper->Accounts($capno,$account_status);
...
public function routedecisionAction(){
	//routine for csh and above auto routing decision
		...
		strpos($detail->routetag,'-EXE-BOD')!== false){	
			...
			if(login_user_role() == 'ALMH' || login_user_role() == 'PRES'){
		...
		$route_orig = $detail->routetag_orig;
		$decision = new Model_CreditDecision();
		$select = $decision->select();
		$select->where('capno like ?',$capno);
		$creDetail= $decision->fetchRow($select);
	...
	if ($this->getRequest()->isPost()) {
		$formData = $this->getRequest()->getPost();
			...
			switch($action){
				...
				$alert =  "You have Disapproved this Account";
				$decision = 'Disapproved';
				break;
				...
				$account_status = $this->view->autoStatusPass($route);	
					...
					if(strpos($route_orig ,'-CRECOM')!== false || strpos($route_orig ,'-EXE-BOD') !== false){ 
				...
				if(login_user_role() == 'PRES'){
					...
					$decision = 'Recommended for Approval - BOD';	
					}
				...
				if(login_user_role() == 'PRES'){
					...
					$decision = 'Recommended for Rejection - BOD';
				}
				else if(login_user_role() == 'CSH'){
				$routex = $this->_helper->AutoRouteAttendance($route);
...
public function routeendorseAction(){
		...
		*/
		...
		if($detail->account_status == 'PRES - R'){
		$this->_redirect('/error/denied/');	
		}
		$capno = $this->_getParam('cap');
		$user = Zend_Auth::getInstance()->getIdentity();
		...
		/**********With RouteTag*********/
			...
			else if(strpos($routetag,'-CRECOM')!== false || strpos($routetag,'-EXE-BOD')!== false){
						$endorse = 'Crecom - InP';
						$nextapprover = 'CRECOM';
...
public function reconAction(){
		...
		*/
		...
		$history = new Model_AccountHistory();
		$days = $this->view->daysDifference(date('Y-m-d'),$history->getApproveDate($capno));
		if($days > 60)			
		{
		$this->_redirect('/error/denied/');		
		}		
...
public function almhcancelAction(){
		$this->view->headScript()->appendFile($this->view->baseUrl().'/js/js_keypress_validation.js');
		$this->view->headScript()->appendFile($this->view->baseUrl().'/js/js_validator.js');
	 	$this->_helper->viewRenderer('almhcancel-form');
		$capno = $this->_getParam('cap');
		if(login_user_role() != 'ALMH'){
			$this->_helper->redirector('denied','error');		
			}
		...
		if ($this->getRequest()->isPost()) {
			$formData = $this->getRequest()->getPost();
				if ($form->isValid($formData)) {
					...
					$history->insert($hdata);
					//End of History
					$this->_helper->Accounts($capno,'ALMH - Cancel');
...
public function aocancelAction(){
		$this->view->headScript()->appendFile($this->view->baseUrl().'/js/js_keypress_validation.js');
		$this->view->headScript()->appendFile($this->view->baseUrl().'/js/js_validator.js');
	 	$this->_helper->viewRenderer('almhcancel-form');
		$capno = $this->_getParam('cap');
		...
		if ($this->getRequest()->isPost()) {
			$formData = $this->getRequest()->getPost();
				if ($form->isValid($formData)) {
					...
					$history->insert($hdata);
					//End of History
					$this->_helper->Accounts($capno,'AO - Cancel');
...
public function recallAction(){
		...
		$this->_helper->viewRenderer('recall-form');
		$capno = $this->_getParam('cap');
		...
		if ($this->getRequest()->isPost()) {
			$formData = $this->getRequest()->getPost();
				...
				$this->_helper->RecallModule($capno);
				$newrecon = capnorecon($capno)+1;
				$newcap = capnoseprecon($capno).$newrecon;	
					...
					'by'=>login_user(),
					'date'=>date("r"),
					'remarks'=>'RECALL '.$form->getValue('submit_remarks'),
					'date_last'=>$historyDetail->date,
					);
					$history->insert($hdata);
					//End of History
...
public function corecallAction(){
		$this->view->headScript()->appendFile($this->view->baseUrl().'/js/js_keypress_validation.js');
		$this->view->headScript()->appendFile($this->view->baseUrl().'/js/js_validator.js');
		//$this->_helper->viewRenderer->setNoRender(true);
		// Recall for MA
		...
		if ($this->getRequest()->isPost()) {
			$formData = $this->getRequest()->getPost();
				if ($form->isValid($formData)) {
					...
					'by'=>login_user(),
					'date'=>date("r"),
					'remarks'=>'CO RECALL '.$form->getValue('submit_remarks'),
					'date_last'=>$historyDetail->date,
					);
					$history->insert($hdata);
					//End of History
...
public function othersAction(){
			...
			if ($this->getRequest()->isPost()) {
				...
				if(login_user_role() == 'CA'){
					...
					$addedRemarks = "System Stamp Recon";
					}
...
public function inboxsubmittedAction() {
		...
		if(login_user_role() == "AO"){
		$this->_helper->viewRenderer('inbox-submitted-ao'); 		
		$select = $accnt->select();
			foreach($statusTable->routeBox('almh_forbooking') as $x){
			$select->orwhere('account_status like ?',$x->status);
			}
...
public function inboxbookedAction() {
	...
	public function inboxcancelAction() {
		...
		if($status == 'cancel'){
		$select->where("account_status = 'CO - Cancel' OR  account_status = 'ALMH - Cancel'");}
...
public function reportAction() {
		...
		$this->_redirect('/autorouting/accountspending');	
		}
			...
			**/
			...
			//else if($formData['button'] == 'Add Requirement'){
				...
				//Additional Deviation
					...
					$mainModel = new Model_BorrowerAccount();
					$mainModel->deviationChk($capno);
...
public function deladddevAction(){
		...
		$where = $table->getAdapter()->quoteInto('id = ?', $id);
		$table->delete($where);
	    $this->_redirect('/index/crawform/cap/'.$capno.'/type/prepare/#additional_deviation');	
}
...
public function getnaremarksAction(){
	$this->_helper->viewRenderer->setNoRender(true);
	...
	$capnoArr = array();
$capnoArr[] = '1100106281000300';
$capnoArr[] = '1100107011000300';
$capnoArr[] = '1100107051000300';
$capnoArr[] = '1100107081000300';
$capnoArr[] = '1100107141000300';
$capnoArr[] = '1100107151000500';
$capnoArr[] = '1100108171000500';
$capnoArr[] = '1100108201000200';
$capnoArr[] = '1100108261000500';
$capnoArr[] = '1100110011000100';
$capnoArr[] = '1100109131000400';
$capnoArr[] = '1100109271000400';
$capnoArr[] = '1100109291000800';
$capnoArr[] = '1100109131000401';
$capnoArr[] = '1100110131000500';
$capnoArr[] = '1100110151000100';
$capnoArr[] = '1100110191000400';
$capnoArr[] = '1100110141000200';
$capnoArr[] = '1100110181000100';
$capnoArr[] = '1100110181000200';
$capnoArr[] = '1100110261000100';
$capnoArr[] = '1100110261001300';
$capnoArr[] = '1100110271000300';
$capnoArr[] = '1100110281000800';
$capnoArr[] = '1100111021000600';
$capnoArr[] = '1100111031000100';
$capnoArr[] = '1100111041000300';
$capnoArr[] = '1100111051000100';
$capnoArr[] = '1100111051000400';
$capnoArr[] = '1100110261001201';
$capnoArr[] = '1100110221000200';
$capnoArr[] = '1100111081000700';
$capnoArr[] = '1100110221000201';
$capnoArr[] = '1100111091000500';
$capnoArr[] = '1100111101000500';
$capnoArr[] = '1100111101000800';
$capnoArr[] = '1100111101001100';
$capnoArr[] = '1100111111000500';
$capnoArr[] = '1100111041000301';
$capnoArr[] = '1100110131000201';
$capnoArr[] = '1100111121000600';
$capnoArr[] = '1100111151000200';
$capnoArr[] = '1100111151000400';
$capnoArr[] = '1100110261000101';
$capnoArr[] = '1100111171000200';
$capnoArr[] = '1100111181000400';
$capnoArr[] = '1100111151000201';
$capnoArr[] = '1100111021000601';
$capnoArr[] = '1100111191000400';
$capnoArr[] = '1100111081000701';
$capnoArr[] = '1100110181000101';
$capnoArr[] = '1100111221000400';
$capnoArr[] = '1100111171000201';
$capnoArr[] = '1100111041000302';
$capnoArr[] = '1100111101000801';
$capnoArr[] = '1100111231000700';
$capnoArr[] = '1100111231000500';
$capnoArr[] = '1100111231001000';
$capnoArr[] = '1100111231001100';
$capnoArr[] = '1100111241000600';
$capnoArr[] = '1100111241000800';
$capnoArr[] = '1100112021000400';
$capnoArr[] = '1100112011000301';
$capnoArr[] = '1100112171000100';
$capnoArr[] = '1100112021000700';
$capnoArr[] = '1100112021000900';
$capnoArr[] = '1100111041000501';
$capnoArr[] = '1100111301000700';
$capnoArr[] = '1100112091000300';
$capnoArr[] = '1100112031000300';
$capnoArr[] = '1100112031000400';
$capnoArr[] = '1100112201000300';
$capnoArr[] = '1100112061000300';
$capnoArr[] = '1100111261000101';
$capnoArr[] = '1100111231000701';
$capnoArr[] = '1100112101000100';
$capnoArr[] = '1100112071000800';
$capnoArr[] = '1100112131000100';
$capnoArr[] = '1100112201001100';
$capnoArr[] = '1100112211000200';
$capnoArr[] = '1100112131001000';
$capnoArr[] = '1100112011000300';
$capnoArr[] = '1100112221000100';
$capnoArr[] = '1100112061000301';
$capnoArr[] = '1100111231000601';
$capnoArr[] = '1100111231001201';
$capnoArr[] = '1100111231001202';
$capnoArr[] = '1100112081000101';
$capnoArr[] = '1100111191000600';
$capnoArr[] = '1100111231000600';
$capnoArr[] = '1100111251000100';
$capnoArr[] = '1100111251000200';
$capnoArr[] = '1100111261000100';
$capnoArr[] = '1100112201001101';
$capnoArr[] = '1100112221000101';
$capnoArr[] = '1100112201000901';
$capnoArr[] = '1100110191000402';
$capnoArr[] = '1100112141000700';
$capnoArr[] = '1100112201000700';
$capnoArr[] = '1100112201000200';
$capnoArr[] = '1100112101000101';
$capnoArr[] = '1100112011000302';
$capnoArr[] = '1100112141000100';
$capnoArr[] = '1100112201000500';
$capnoArr[] = '1100112211000700';
$capnoArr[] = '1100110191000401';
$capnoArr[] = '1100112211000800';
$capnoArr[] = '1100112131001600';
$capnoArr[] = '1100112151000300';
$capnoArr[] = '1100112131001300';
$capnoArr[] = '1100112221000200';
$capnoArr[] = '1100112201000900';
$capnoArr[] = '1100112171000500';
$capnoArr[] = '1100112171000300';
$capnoArr[] = '1100112131001700';
$capnoArr[] = '1100112131000400';
$capnoArr[] = '1100112141000200';
$capnoArr[] = '1100112161000500';
$capnoArr[] = '1100112211000400';
$capnoArr[] = '1100112081000100';
$capnoArr[] = '1100112131001100';

	
	$capArr = array();
	foreach($capnoArr as $x){
		
		$select = $table->select();
		$select->where("capno like ?",$x);
		$select->where("status = 'CO - Cancel' OR status = 'CO - NA'");
		$detail = $table->fetchRow($select);
		
		$capArr[$x]['capno'] = $detail->capno;
		$capArr[$x]['caprecon'] = capnorecon($detail->capno);
		$capArr[$x]['remarks'] = $detail->remarks;
		
		
	}
	$data_headers[] = 'Capno';
	$data_headers[] = 'CapnoRecon';
	$data_headers[] = 'Remarks';
	
	$html_string='<table>';
	$html_string.='<tr><td>'.implode('</td><td>',$data_headers).'</td></tr>';
	foreach($capArr as $k=>$v){
...
public function riskexcelAction(){
	...
	$startdate = "2010-10-01 01:00:00";	// of the morning 
	$enddate = "2011-02-20 24:00:00";	 // of the evening
	/*
	foreach($statusTable->routeBox('approve') as $x){
	...
	$data_headers[] = 'Approval Status';
	$data_headers[] = 'Capno Extension';
	$data_headers[] = 'Process Type';
	$data_headers[] = 'Name';
	$data_headers[] = 'Amount Financed';
	$data_headers[] = 'Account Officer';
	$data_headers[] = 'Last User';
	$data_headers[] = 'Credit Score';
	$data_headers[] = 'PN Number';
	$data_headers[] = 'Account Status';
	...
	$statusTable = new Model_Admin_AccountStatus();
				...
				$condition = $select->getPart(Zend_Db_Select::WHERE);
				$select = $table->select()->from($table,$arrayCol)->order("date DESC");	
				$select->where("date between '$startdate'  and '$enddate'");
				$select->where(arrayString($condition));
	$data_array = $table->fetchAll($select)->toArray();			
	/*****End of Using Report Table*****/
	$count = 0;
	$borrower = new Model_BorrowerAccount();
	foreach($data_array as $key => $val){

		$detail = $borrower->fetchRowModel($data_array[$key]['capno']);
		$bookingDetail = $booking->fetchRowModel($data_array[$key]['capno']);
		$dc = '';
		$mo = '';
		...
		$data_array[$key]['Last User'] = $this->view->viewMa($history->getLastUserbyStatus($detail->capno,$detail->account_status));
		$data_array[$key]['score_tag'] = $scoreTag;
		$data_array[$key]['pn_number'] = $bookingDetail->pn_number;
		$data_array[$key]['Status'] = $detail->account_status;
...
public function armaeexcelAction(){
	// armae-excel new route report
	//Armae monthly report!!!!!
	$this->_helper->viewRenderer->setNoRender(true);
	ini_set('max_execution_time', 0);
	$table = new Model_BorrowerAccount();
	$select = $table->select();
	...
	$bookAuto = new Model_Booking_BookingAuto();
	
	
	$historyTable = new Model_AccountHistory();
	$statusTable = new Model_Admin_AccountStatus();
	
	$startdate = "2011-02-01:00:00";	// of the morning 
	//$enddate = "2010-05-16 24:00:00";	 // of the evening
	$enddate = "2011-03-01 24:00:00";	 // of the evening
	//$enddate = "2010-01-01 24:00:00";	 // of the evening

	$select = $historyTable->select();
	
	foreach($statusTable->routeBox('armae_report') as $x){
			$select->orwhere('status like ?',$x->status);
			}
	$condition = $select->getPart(Zend_Db_Select::WHERE);
	$select = $historyTable->select();
	$select->where("date between '$startdate' and '$enddate'");
	$select->where(arrayString($condition));
	$historyDetail = $historyTable->fetchAll($select);
	
	//$select->where("status like 'CO%' OR account_status = 'Approved' OR account_status = 'Rejected'");

	$capnoArr = array();
	
	foreach($historyDetail as $x){
		if(!in_array($x,$capnoArr)){
		$capnoArr[] = $x->capno;
		}
	}
	/******************************/
	...
	foreach($data_array as $key => $val){
		...
		if($status == 'E'){
			...
			$emp_industry = $this->_helper->ExcelFetch->getEmpFields($capbasis, 'values','empindustry');
		...
		$data_array[$key]['capbasisEmpIndustry'] = $emp_industry;
		...
		$data_array[$key]['dateStatus'] = $history->getLastDatebyStatus($data_array[$key]['capno'],$data_array[$key]['account_status']);
		$data_array[$key]['byStatus'] = $history->getLastUserbyStatus($data_array[$key]['capno'],$data_array[$key]['account_status']);
		$data_array[$key]['remarks-new'] = $history->getLastRemarks($data_array[$key]['capno'],$data_array[$key]['account_status']);
		$counterCraw = 0;
		foreach($crawformApproval->getApprovals($data_array[$key]['capno']) as $x){
		$data_array[$key][$counterCraw]	 = $x->decision.' - '.$this->view->viewMa($x->approved_by);
		$counterCraw++;
		}
		$data_array[$key]['pnNumber'] = $bookAuto->getPn($data_array[$key]['capno']);		
	}
...
public function armaeexcel2Action(){
	// armae-excel new route report
	$this->_helper->viewRenderer->setNoRender(true);
	ini_set('max_execution_time', 0);
	$table = new Model_BorrowerAccount();
	...
	$startdate = "2010-10-01 01:00:00";	// of the morning 
	//$enddate = "2010-05-16 24:00:00";	 // of the evening
	$enddate = "2010-10-31 24:00:00";	 // of the evening
	//$enddate = "2010-01-01 24:00:00";	 // of the evening
	/*
	$select = $historyTable->select();
	foreach($statusTable->routeBox('armae_report_booked') as $x){
			$select->orwhere('status like ?',$x->status);
			}
	$condition = $select->getPart(Zend_Db_Select::WHERE);
	$select = $historyTable->select();
	...
	*/
	...
	$capnoArr[] = '1100110211001400';
	$capnoArr[] = '1100110191000300';
...
public function pnexcelAction(){
	...
	$startdate = "2010-08-01 01:00:00";	// of the morning 
	$enddate = "2010-12-30 24:00:00";	 // of the evening
	/*
	foreach($statusTable->routeBox('approve') as $x){
	$select->orwhere('status like ?',$x->status);
	$condition = $select->getPart(Zend_Db_Select::WHERE);
	...
	$select = $table->select()->from($table,$arrayCol);
	//$select->where("status like ?","Booked");
	$select->where("bookdate between '$startdate' and '$enddate'");
	$select->order('bookdate');		
	//$select->where(arrayString($condition));
...
public function approveexcelAction(){
	//Irene
	$this->_helper->viewRenderer->setNoRender(true);
	ini_set('max_execution_time', 0);
	...
	$startdate = "2010-11-01 01:00:00";	// of the morning 
	$enddate = "2010-11-30 24:00:00";	 // of the evening
	/*
	foreach($statusTable->routeBox('approve') as $x){
	$select->orwhere('status like ?',$x->status);
	$condition = $select->getPart(Zend_Db_Select::WHERE);
	...
	$data_headers[] = 'Capno';
	$data_headers[] = 'CapnoRecon';
	$data_headers[] = 'Account Status 2';
	$data_headers[] = 'Name';
	$data_headers[] = 'Decision';
	$data_headers[] = 'ScoreTag';
	...
	foreach($data_array as $key => $val){
		$detail = $borrower->fetchRowModel($data_array[$key]['capno']);
		$route = $this->_helper->AutoRoute($data_array[$key]['capno'],$data_array[$key]['score_tag']);
		$data_array[$key]['req_cap'] = $data_array[$key]['capno'];
		$data_array[$key]['req_caprecon'] = capnorecon($data_array[$key]['capno']);
		$data_array[$key]['req_acctstatus2'] = $detail->account_status2;
		$data_array[$key]['req_name'] = $detail->borrower_lname.', '.$detail->borrower_fname.' '.$detail->borrower_mname;
		$data_array[$key]['req_statusdate'] = $this->_helper->ExcelFetch->getApprove($data_array[$key]['capno']);
		$data_array[$key]['score_tag'] = $detail->score_tag;
	}
	//print_r($data_array[6]);
...
public function mareportAction(){
	$this->_helper->viewRenderer->setNoRender(true);
	ini_set('max_execution_time', 0);
	...
	$startdate = "2010-09-01 01:00:00";	// of the morning 
	$enddate = "2010-11-30 24:00:00";	 // of the evening
	...
	$data_headers[] = 'Name';
	$data_headers[] = 'Brand';
	$data_headers[] = 'LoanTerm';
	$data_headers[] = 'Dealer';
	$data_headers[] = 'Downpayment %';
	$data_headers[] = 'Downpayment Actual';
	$data_headers[] = 'Approved By';
	$data_headers[] = 'Approval Level';
	$data_headers[] = 'Recommend By';
	$count = 0;
	$history = new Model_AccountHistory();
	foreach($data_array as $key => $val){
		$route = $this->_helper->AutoRoute($data_array[$key]['capno'],$data_array[$key]['score_tag']);
		$data_array[$key]['req_cap'] = $data_array[$key]['capno'];
		$data_array[$key]['req_caprecon'] = capnorecon($data_array[$key]['capno']);
		$data_array[$key]['req_acctstatus'] = $data_array[$key]['account_status'];
		$data_array[$key]['req_acctstatus2'] = $data_array[$key]['account_status2'];
		$data_array[$key]['req_name'] = $data_array[$key]['borrower_lname'].', '.$data_array[$key]['borrower_fname'].' '.$data_array[$key]['borrower_mname'];
		$data_array[$key]['req_veh_brand'] = $data_array[$key]['veh_brand'];	
		$data_array[$key]['req_loanterm'] = $data_array[$key]['loanterm'];	
		$data_array[$key]['req_dealer'] = $data_array[$key]['dealer'];	
		$data_array[$key]['req_downpayment_percent'] = $data_array[$key]['downpayment_percent'];	
		$data_array[$key]['req_downpayment_actual'] = $data_array[$key]['downpayment_actual'];	
		$data_array[$key]['req_routetag'] = $this->view->approvalLevel($data_array[$key]['routetag'],'level');	
		$data_array[$key]['req_routetag_orig'] = $this->view->approvalLevel($route,'level');	
		//$data_array[$key]['req_recommend'] = $history->chkRecommended($data_array[$key]['capno']);
		//$data_array[$key]['req_notrecommend'] = $history->chknotRecommended($data_array[$key]['capno']);
		$data_array[$key]['req_chkrecommend'] = $history->ifRecommend($data_array[$key]['capno']);
		$data_array[$key]['req_caprecon'] = capnorecon($data_array[$key]['capno']);
		}
		//print_r($data_array[6]);
...
public function excelAction(){
	...
	$startdate = "2010-07-01 01:00:00";	// of the morning 
	//$enddate = "2010-05-16 24:00:00";	 // of the evening
	$enddate = "2010-09-30 24:00:00";	 // of the evening
	//$enddate = "2010-01-01 24:00:00";	 // of the evening
...
public function excelempindustryAction(){
	$this->_helper->viewRenderer->setNoRender(true);
	ini_set('max_execution_time', 0);
	$table = new Model_BorrowerAccount();
	$select = $table->select();
	...
	$startdate = "2009-06-16 01:00:00";	// of the morning 
	//$enddate = "2010-05-16 24:00:00";	 // of the evening
	$enddate = "2010-12-30 24:00:00";	 // of the evening
	//$enddate = "2010-01-01 24:00:00";	 // of the evening
...
public function excelciAction(){
	...
	$array = array();
$array[] ='11011240900800';
$array[] ='11012010900400';
	...
	foreach($data_array as $key => $val){
	$capbasis = getHighest($data_array[$key]['capno']);
	$cibackgrd = $this->_helper->ExcelFetch->getCiFields($capbasis, 'background');	
	$cvbackgrd = $this->_helper->ExcelFetch->getCvFields($capbasis, 'background');	
		
	$data_array[$key]['custom name'] = $data_array[$key]['borrower_lname'].', '.$data_array[$key]['borrower_fname'].' '.$data_array[$key]['borrower_mname'];
	$data_array[$key]['cv_background'] = $cvbackgrd;
	$data_array[$key]['ci_background'] = $cibackgrd;
	$data_array[$key]['checking'] = $history->chkCVCI($data_array[$key]['capno']);
	}
	...
	$data_headers[]  = 'Custom Name';
	$data_headers[] ='CV Background';
	$data_headers[] ='CI Background';
	$data_headers[] ='Checking';
...
public function excel2Action(){
	...
	$array = array();
$array[] ='11012080901100';
$array[] ='11012140900500';
$array[] ='11001041000100';
$array[] ='1100101081000600';
$array[] ='1100101191001100';
$array[] ='1100101201001100';
$array[] ='1100102251000800';
$array[] ='1100103031000800';
$array[] ='1100103231000900';
$array[] ='1100104081000200';
$array[] ='1100104121001400';
$array[] ='1100104131000500';
$array[] ='1100104131000600';
$array[] ='1100104131000800';
$array[] ='1100104141000200';
$array[] ='1100104161000400';
$array[] ='1100104161000700';
$array[] ='1100104191001000';
$array[] ='1100104201000700';
$array[] ='1100104271000100';
$array[] ='1100104281000100';
$array[] ='1100104281000500';
$array[] ='1100104291000400';
$array[] ='1100104301000600';
$array[] ='1100104301000700';
$array[] ='1100104301000800';
$array[] ='1100104301000900';
$array[] ='1100104301001300';
$array[] ='1100105061000100';
$array[] ='1100105131000900';
$array[] ='1100105141000200';
$array[] ='1100105251000300';
$array[] ='1100105311000400';
$array[] ='1100106021000100';
$array[] ='1100106021000500';
$array[] ='1100106101000100';
$array[] ='1100106161000600';
$array[] ='1100106171000100';
$array[] ='1100106171000400';
$array[] ='1100106171000500';
$array[] ='1100106181000400';
$array[] ='1100106211000200';
$array[] ='1100106071001000';
$array[] ='1100106151000100';
$array[] ='1100104141000300';
$array[] ='1100104121001100';
$array[] ='1100106231000500';
$array[] ='1100107021000200';
$array[] ='1100107051001200';
$array[] ='1100107051001600';
$array[] ='1100107091000800';
$array[] ='1100107051001801';
$array[] ='1100107151000200';
$array[] ='1100107161000100';
$array[] ='1100107161001200';
$array[] ='1100107081000400';
$array[] ='1100107161001100';
$array[] ='1100107201000200';
$array[] ='1100107201001300';
$array[] ='1100106211000901';
$array[] ='1100106181000601';
$array[] ='1100107211000100';
$array[] ='1100106161000501';
$array[] ='1100107011000301';
$array[] ='1100107231000300';
$array[] ='1100106111000201';
$array[] ='1100107231000500';
$array[] ='1100107271000100';
$array[] ='1100107261000100';
$array[] ='1100107271001000';
$array[] ='1100107271002000';
$array[] ='1100107291000500';
$array[] ='1100107281000600';
$array[] ='1100108021000300';
$array[] ='1100106171000200';
$array[] ='1100108031000100';
$array[] ='1100108051000300';
$array[] ='1100108041000100';
$array[] ='1100108061000100';
$array[] ='1100108091000400';
$array[] ='1100108091000600';
$array[] ='1100108091001100';
$array[] ='1100108111000200';
$array[] ='1100108121000300';
$array[] ='1100107051000301';
$array[] ='1100107151000501';
$array[] ='1100108191000400';
$array[] ='1100108201000300';
$array[] ='1100108201000600';
$array[] ='1100108201000700';
$array[] ='1100108231000300';
$array[] ='1100108091000300';
$array[] ='1100108201000801';
$array[] ='1100108271000300';
$array[] ='1100109071000100';
$array[] ='1100107161001001';
$array[] ='1100109081000100';
$array[] ='1100109011000200';
$array[] ='1100108051000100';
$array[] ='1100107151000700';
$array[] ='1100108241000400';
$array[] ='1100107201000300';
$array[] ='1100109141000100';
$array[] ='1100109141001400';
$array[] ='1100109141001300';
$array[] ='1100109031000100';
$array[] ='1100108191000200';
$array[] ='1100108191000501';
$array[] ='1100107021000100';
$array[] ='1100109171000100';
$array[] ='1100107151000300';
$array[] ='1100109201000500';
$array[] ='1100109201000100';
$array[] ='1100108201000202';
$array[] ='1100106221000800';
$array[] ='1100108091000100';
$array[] ='1100108091000200';
$array[] ='1100107231000600';
$array[] ='1100109201000400';
$array[] ='1100109091001000';
$array[] ='1100109141000201';
$array[] ='1100106031000400';
$array[] ='1100106181000500';
$array[] ='1100108251000402';
$array[] ='1100108061000202';
$array[] ='1100107071000600';
$array[] ='1100109161000101';
$array[] ='1100107051001500';
$array[] ='1100108161000401';
$array[] ='1100107271000301';
$array[] ='1100107011000400';
$array[] ='1100107011000100';
$array[] ='1100108191000300';
	...
	$this->_helper->viewRenderer->setNoRender(true);
	echo compare_date('2010-09-10','2010-09-15');
	}
...
function updateLoanAmount2($capno){
	...
	$data = array(
	'amountloan2'=>moneyconvert($total_bankproper+$detail->amountloan),
	);
	$where = "capno like '$capno'";
	$table->update($data,$where);
...
function compare_date($begin, $end) {
