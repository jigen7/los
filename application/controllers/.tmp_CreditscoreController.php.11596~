...
class CreditscoreController extends Zend_Controller_Action
{
	...
	public function outputAction(){
		...
		$this->view->prodType = $prodType;	
		$sesid = $this->_getParam('sesid');
		$process = $this->_getParam('act'); // score || view
		...
		$accountArr[$counter]['data'] = $this->_helper->CreditScore->scoreMultipleAccount($accounts);
		$counter++;
		}
		preint_r($accountArr);		
...
public function output2Action()
	...
	public function addmodelocsAction()
	{
		...
		if($this->getRequest()->isPost()) {
			...
			$checkdev = $ocsmodel->checkDevCat($deviation, $namever);
			if($checkdev || $description == "" || $checking == ""){
				if($checkdev) $this->view->errorDeviation = "Deviation exist"; 
				if($checking == "") $this->view->errorDescription = "Description please"; 
				if($checking == "") $this->view->errorChecking = "Please select"; 
