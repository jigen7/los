...
class Model_Admin_TurnAroundHoliday extends Zend_Db_Table_Abstract{
	...
	/**
	*	Created by: Joseph Lacerna(2011-01-28)
	*/
	public function dateTimeDiff($end,$start){
	$end = rtrim(ltrim($end));
	$start = rtrim(ltrim($start));
		//check if not null
	if($end!=null AND $start!=null){
		...
		//output
		$output = "";
		if($end>=$start AND $pg_end['year'] != '1970' AND $pg_start['year'] != '1970'){
			//set holiday list
			$hl = $this->fetchAll();
			//additional date
			$add = array(
			...
			if($day_gap>=1){
				...
				echo $pd_start['hour'].":".$pd_start['minute'].":".$pd_start['second'];
		...
		return $output;//"<br/>Start[{$start}] End[{$end}]  <br/> Less: {$less}<br/> Start[{$pd_start['hour']}]:[{$pd_start['minute']}]:[{$pd_start['second']}]<br/> End[{$pd_end['hour']}]:[{$pd_end['minute']}]:[{$pd_end['second']}]<br/><br/> Start[{$pd_start['year']}]-[{$pd_start['day']}]-[{$pd_start['month']}]<br/> End[{$pd_end['year']}]-[{$pd_end['day']}]-[{$pd_end['month']}]";
