 ...
 * @package LOS
 */
class Model_AutoRouting_AccountsCrecom extends Zend_Db_Table {
	protected $_name="autorouting.accounts_crecom";
	...
	public function chkChairmanDecisionNormal($capno){
		$select = $this->select();
		$select->where('capno like ?',$capno);
		$select->where("decision = 'Approved' or decision = 'Disapproved'");
		$select->where('type like ?','chairman');
