...
class pricelistchaintype
{

	var $queryVars;
	function pricelistchaintype($queryVars)
	{
	...
	$sql ="Select distinct(type) FROM select_chain_vehicle_new where month = '$month' AND year = '$year' AND dealer = '$dealer' AND brand = '$brand' AND status = 'approved' AND unit = '$unit'";
	$res = pg_query($sql);  
