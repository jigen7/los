...
class pricelistchainid
{

	var $queryVars;
	function pricelistchainid($queryVars)
	{
	...
	$sql ="Select distinct(id) FROM select_chain_vehicle_new where month = '$month' AND year = '$year' AND dealer = '$dealer' AND brand = '$brand' AND status = 'approved' AND unit = '$unit'";
	$res = pg_query($sql);  
